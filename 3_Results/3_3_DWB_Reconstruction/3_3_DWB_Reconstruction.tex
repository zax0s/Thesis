...
Another considerable limitation, that has not been considered in this study, is subject voluntary and involuntary motion. Inevitably these types of motion will be present in dynamic studies, especially from DWB protocols which involve frequent bed movements. In dynamic reconstruction these need to be accounted for within the reconstruction process, to avoid image artefacts and modelling errors.
But when considering the whole-body, there are many types of complex motion that take place during the duration of a dynamic study. These include respiratory motion, cardiac motion, head movements , bowel motion etc. Estimation of all these types of motion is a relatively complex task.
Many data driven approaches have been used for gating and rigid motion estimation, but their use for complex elastic motion is still actively researched.
Machine learning frameworks have been recently proposed for data driven estimation of elastic motion from PET data alone~\ref{Zhou2021}. 

Use of specialised MR sequences in PET/MR imaging has been previously suggested and successfully used for complex elastic motion estimation, but those can be time consuming. In DWB imaging where delays between individual body position acquisitions have a strong impact on the estimation of kinetic parameters, there can be no additional sacrifice of time for MR sequences other than those necessary for attenuation correction on each bed position. 
