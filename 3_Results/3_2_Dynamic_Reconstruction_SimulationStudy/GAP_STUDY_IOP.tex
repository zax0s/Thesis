...
\textcolor{blue}{For this reason the DWB-4 protocol was designed to evaluate estimation of $\boldsymbol{K_1}$ parametric images for bed positions of DWB protocols that are covered by the single bed dynamic phase. Accurate $K_1$ estimation requires correction for the blood fraction ratio, as shown in equation~\ref{eqn:SpectralCPET_AllEquations}. We intentionally ignored this term for estimation of parametric images to avoid the introduction of noise by the voxel-to-voxel division with the blood fraction image. In this project $\boldsymbol{K_1}$ parametric images were estimated using}
