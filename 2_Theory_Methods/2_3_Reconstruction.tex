...
%using voxels with the $rect$ function as
%\begin{equation}
%   g(\bm{r};\bm\lambda) = \sum_{j=1}^{n_{j}} \lambda_j {rect}(\bm{r}-\bm{r}_j)  \\, 
%\end{equation}
with an ${n_{j}}$ number of voxels%with $r_j$ coordinates 
...
%where
\begin{equation}
   \bm{\lambda}_t = \big\{ {\lambda_{tj}| j=1,\cdots,n_j} \big\} \\.
\end{equation}
...
%\end{figure} 
Similarly, back-projectors using the same methods operation assign to voxels in the \gls{fov} the contribution from each \gls{lor} bin.
...
\begin{itemize}
    ...
    \item $X_{ij}$ is the geometrical projection matrix entry
    \item $H_{jk}$ is the resolution modelling matrix entry, applied as a convolution in image space to account for different effects impacting resolution (positron range, detector size, etc.)
...
The data of each frame can then be written as
\begin{equation}
   \bm{y}_t = \big\{ {y_{ti}| i=1,\cdots,n_i} \big\} \\.
\end{equation}
%
...
\begin{equation}
   \bm{\theta}_j = \big\{ {\theta_{pj}| p=1,\cdots,n_p} \big\} \\.
\end{equation}
...
\begin{equation}
    lnL(\bm{\theta}|\bm{y})  = \sum_{t=1}^{n_t}\sum_{i=1}^{n_i}\left[ y_{ti} ln(\sum_{j=1}^{n_j} P_{ij} f_{tj}(\bm\theta) + b_{ti}) -\sum_{j=1}^{n_j} P_{ij} f_{tj}(\bm\theta) - b_{ti} - ln({y_{ti}}!) \right] \\, \\
...
Similar to~\ref{eq:Complete_Data_Expectation1} and~\ref{eq:Complete_Data_Expectation2}, the conditional expectation of the complete data can be written using the generic dynamic model $f()$ as 
\begin{equation}
